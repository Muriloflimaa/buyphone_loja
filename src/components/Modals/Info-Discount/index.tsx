...
export default function InfoDiscount() {
    const handleCloseModalInfoDiscount = () => {
        return document.getElementById('modal-info-discount')?.classList.remove('modal-open')
    }
    return (
        <>
            <div className='modal modal-bottom sm:modal-middle' id="modal-info-discount">
                <div className='h-screen sm:h-auto relative'>
                    <div className={`sm:p-10 h-full max-h-full max-w-full sm:w-[650px] modal-box flex flex-col items-center bg-white ${styles.background}`}>
                        ...
                        <div className="w-[250px] modal-action absolute bottom-0 mb-10 sm:mb-auto sm:static">
                            <label onClick={handleCloseModalInfoDiscount} className="btn bg-info border-info text-white w-full">Entendi</label>
                        </div>
                    </div>
