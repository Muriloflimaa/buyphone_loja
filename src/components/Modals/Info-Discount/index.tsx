...
export default function InfoDiscount() {
    const handleCloseModal = () => {
        return document.getElementById('modal-open-cadastre')?.classList.remove('modal-open')
    }
    return (
        <>
            <input type="checkbox" id="modal-info-discount" className="modal-toggle" />
            <div className='modal modal-bottom sm:modal-middle z-[9999]' id="modal-open-cadastre">
                <div className='h-screen sm:h-auto relative'>
                    <div className={`sm:p-10 h-full max-h-full max-w-full sm:w-[650px] modal-box flex flex-col items-center bg-white ${styles.background}`}>
                        <label onClick={handleCloseModal} className="btn border-transparent text-xl text-[#CCCCCC] bg-inherit btn-circle absolute right-3 top-3">✕</label>
                        ...
                        <div className="w-[250px] modal-action absolute bottom-0 mb-10 sm:mb-auto sm:static">
                            <label onClick={handleCloseModal} className="btn bg-info border-info text-white w-full">Entendi</label>
                        </div>
                    </div>
